...
from cli_agent_manager.services.session_service import delete_session, list_sessions


@click.command()
@click.option("--all", "shutdown_all", is_flag=True, help="Shutdown all tron sessions")
...
def shutdown(shutdown_all, session):
    ...
    if not sessions_to_shutdown:
        click.echo("No tron sessions found to shutdown")
        return
