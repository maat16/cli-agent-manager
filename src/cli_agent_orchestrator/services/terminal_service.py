...
from cli_agent_manager.clients.database import create_terminal as db_create_terminal
from cli_agent_manager.clients.database import delete_terminal as db_delete_terminal
from cli_agent_manager.clients.database import (
    get_terminal_metadata,
    update_last_active,
)
from cli_agent_manager.clients.tmux import tmux_client
from cli_agent_manager.constants import SESSION_PREFIX, TERMINAL_LOG_DIR
from cli_agent_manager.models.provider import ProviderType
from cli_agent_manager.models.terminal import Terminal, TerminalStatus
...
def create_terminal(
    ...
    try:
        ...
        try:
            logger.info(f"Initializing {provider} provider for terminal {terminal_id}")
            provider_instance = provider_manager.create_provider(
                provider, terminal_id, session_name, window_name, agent_profile
            )
            provider_instance.initialize()
            logger.info(f"Successfully initialized {provider} provider for terminal {terminal_id}")
        except Exception as e:
            logger.error(f"Provider initialization failed for {provider}: {e}")
            # Cleanup on failure
            try:
                if new_session:
                ...
                logger.error(f"Cleanup failed after provider initialization error: {cleanup_error}")
            raise
