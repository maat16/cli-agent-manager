...
- `allowedTools` (array): Whitelist of tools (e.g., `["@builtin", "@tron-mcp-server"]`)
...
```bash
# From local file
tron install ./my-agent.md

# From URL
tron install https://example.com/agents/my-agent.md
...
tron includes these built-in profiles:
- `code_supervisor`: Coordinates development tasks
- `developer`: Writes code
- `reviewer`: Performs code reviews

View the [agent_store directory](https://github.com/maat16/cli-agent-manager/tree/main/src/cli_agent_manager/agent_store) for examples.
