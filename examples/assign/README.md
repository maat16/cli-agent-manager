...
All agents require the **tron-http-server** configuration in their frontmatter to access orchestration tools:
...
This configuration provides three HTTP-based orchestration tools:
...
1. Get your terminal ID: my_id = tron_TERMINAL_ID
...
./examples/assign/test-example.sh
```
...
**Terminal 2 - Install and Launch Agents:**
```bash
# Install the HTTP-based agent profiles for Kiro CLI
...
# Launch the HTTP-based supervisor with Kiro CLI
tron launch --agents analysis_supervisor_http --provider kiro_cli
```

**Terminal 2 - Run Example (in the supervisor agent terminal):**

Copy and paste this exact example:
...
1. **Supervisor assigns 3 data analysts** (parallel execution)
2. **Supervisor handoffs to report generator** (sequential, waits for completion)  
3. **Data analysts send results back** via send_message (async callbacks)
4. **Supervisor combines everything** into a final comprehensive report

### Expected Output

The supervisor will orchestrate the workflow and produce a final report combining:
- Statistical analysis from 3 parallel data analysts
- Professional report template from report generator
...
```bash
# List all tmux sessions to see agents
tmux list-sessions

# You should see sessions like:
# tron-analysis_supervisor_http-abc123: 1 windows
# tron-data_analyst-def456: 1 windows  
# tron-data_analyst-ghi789: 1 windows
# tron-report_generator-jkl012: 1 windows
```

**Attach to Agent Sessions (Optional):**
```bash
# Watch a data analyst working
tmux attach -t tron-data_analyst-[session-id]
...
```
Supervisor checks tron_TERMINAL_ID (e.g., "super123")
